...
function GuildRoster:OnSave(eType)
	...
	local tSavedData =
	{
		...
		bShowMaxLevel = self.bShowMaxLevel,
	}
...
function GuildRoster:OnRestore(eType, tSavedData)
	...
	if tSavedData.bShowMaxLevel ~= nil then
		self.bShowMaxLevel = tSavedData.bShowMaxLevel
	end
...
function GuildRoster:Initialize(wndParent)
	...
	self.tWndRefs.wndMain:FindChild("ShowMaxLevel"):SetCheck(self.bShowMaxLevel)
...
function GuildRoster:FullRedrawOfRoster()
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() or not self.tWndRefs.wndMain:IsVisible() or not self.tWndRefs.wndMain:GetData() then
		return
	end
	...
	self:BuildRosterList(guildCurr, self:SortRoster(self.tWndRefs.wndMain:FindChild("RosterHeaderContainer"):GetData(), "RosterSortBtnOnline"))
...
function GuildRoster:OnGuildRoster(guildCurr, tRoster) -- Event from CPP
	if self.tWndRefs.wndMain and self.tWndRefs.wndMain:IsValid() and guildCurr == self.tWndRefs.wndMain:GetData() then
		...
		self:BuildRosterList(guildCurr, self:SortRoster(tRoster, "RosterSortBtnOnline")) -- "RosterSortBtnName" is the default sort method to use	
...
function GuildRoster:BuildRosterList(guildCurr, tRoster)
	...
	for key, tCurr in pairs(tRoster) do
		...
		if self.bShowMaxLevel then
			bShowLevel = 50;
		end
		
		if tCurr.nLevel > bShowLevel or bShowLevel == tCurr.nLevel then
			if self.bShowOffline or tCurr.fLastOnline == 0 then
				...
				if tCurr.ePathType == PlayerPathLib.PlayerPathType_Soldier then
					nPathIcon = "IconSprites:Icon_MapNode_Map_Soldier_Accepted"
				elseif tCurr.ePathType == PlayerPathLib.PlayerPathType_Settler then
					nPathIcon = "IconSprites:Icon_MapNode_Map_Settler_Accepted"
				elseif tCurr.ePathType == PlayerPathLib.PlayerPathType_Explorer then
					nPathIcon = "IconSprites:Icon_MapNode_Map_Explorer_Accepted"
				elseif tCurr.ePathType == PlayerPathLib.PlayerPathType_Scientist then
					nPathIcon = "IconSprites:Icon_MapNode_Map_Scientist_Accepted"
				end
				...
				wndGrid:SetCellImage(iCurrRow, 5, nClassIcon)
				wndGrid:SetCellImage(iCurrRow, 6, nPathIcon)
...
function GuildRoster:OnMaxLevelBtn(wndHandler, wndControl)
	self.bShowMaxLevel= wndControl:IsChecked()
...
function GuildRoster:SortRoster(tArg, strLastClicked)
	...
	if self.tWndRefs.wndMain:FindChild("RosterSortBtnOnline"):IsChecked() then
		table.sort(tResult, function(a,b) return (a.fLastOnline > b.fLastOnline) end)
	elseif self.tWndRefs.wndMain:FindChild("RosterSortBtnName"):IsChecked() then
