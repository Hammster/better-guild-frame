...
function Circles:OnGuildRoster(guildCurr, tRoster) -- Event from CPP
	...
	if guildCurr == self.tWndRefs.wndMain:GetData() then -- Since Circles and Guild can be up at the same time
		...
		self:BuildRosterList(guildCurr, self:SortRoster(tRoster, "RosterSortBtnOnline")) -- "RosterSortBtnName" is the default sort method to use Edit: nope RosterSortBtnOnline is better
	end
...
function Circles:SortRoster(tArg, strLastClicked)
	...
	if wndHeaderContainer:FindChild("RosterSortBtnOnline"):IsChecked() then
		table.sort(tResult, function(a,b) return (a.fLastOnline > b.fLastOnline) end)
	elseif wndHeaderContainer:FindChild("RosterSortBtnName"):IsChecked() then
